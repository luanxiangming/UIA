...
IPA=idevice[:ipa]
IOS_ARGS="APP=#{BUNDLE_ID} BUNDLE_ID=#{BUNDLE_ID} DEVICE_TARGET=#{IOS_DEVICE} DEVICE_ENDPOINT=#{ENDPOINT} #{ARGV.join(' ')}"
...
if target == 'android'
  ...
  system("#{ANDROID_ARGS} calabash-android run #{APK} -p #{target}")
elsif target == 'ios'
  ...
  system("#{IOS_ARGS} cucumber -p #{target}")
else
